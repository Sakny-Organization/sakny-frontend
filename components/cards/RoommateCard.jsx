...
const RoommateCard = ({ roommate }) => {
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.4, ease: [0.23, 1, 0.320, 1] }} whileHover={{ y: -8 }} className="group bg-white rounded-xl shadow-sm hover:shadow-hover border border-gray-100 overflow-hidden transition-all duration-300 cursor-pointer flex flex-col h-full" onClick={handleViewProfile}>
      ...
      <div className="relative h-64 w-full overflow-hidden">
        ...
        {/* Online Status */}
        {roommate.isOnline && (<div className="absolute top-3 left-3 flex items-center gap-1.5 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full shadow-sm">
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {roommate.tags.slice(0, 3).map((tag, idx) => (<motion.span key={idx} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.1, duration: 0.3 }} className="bg-gray-50 text-gray-600 border border-gray-200 px-2.5 py-1 rounded-full text-[11px] font-medium tracking-wide">
              {tag}
            </motion.span>))}
        </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-100 mt-auto">
          ...
          <div className="flex gap-3">
            <motion.button onClick={handleSave} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} className={`w-10 h-10 rounded-full flex items-center justify-center border transition-all ${isSaved
            ? 'bg-black border-black text-white hover:bg-gray-800'
            : 'bg-white border-gray-200 text-gray-400 hover:border-black hover:text-black'}`} title={isSaved ? "Unsave Profile" : "Save Profile"}>

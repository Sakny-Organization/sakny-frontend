...
const Navbar = () => {
    ...
    return (
        ...
        <div className="bg-white/80 backdrop-blur-md border border-gray-200/60 rounded-2xl px-6 py-3 flex items-center justify-between shadow-sm transition-all duration-300 hover:shadow-md supports-[backdrop-filter]:bg-white/60">
          {/* Logo */}
          <div className="flex items-center gap-3">
            <Link to="/" className="flex items-center gap-2.5 group">
              <div className="size-9 bg-black text-white rounded-xl flex items-center justify-center shadow-lg transform transition-transform group-hover:scale-105 duration-300">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
                  <path d="M12 3L4 9v12h5v-7h6v7h5V9l-8-7z" />
                </svg>
              </div>
          ...
          <div className="hidden md:flex items-center gap-8">
            {isAuthenticated ? (<>
                ...
                return (<Link key={link.path} to={link.path} className={`flex items-center gap-2 text-sm font-medium transition-all duration-200 relative px-3 py-1.5 rounded-lg hover:bg-gray-100/50 ${isActive(link.path)
                      ...
                      {link.icon}
                      {link.name}
                      {count > 0 && (<span className="absolute -top-1 -right-1 h-4 min-w-[16px] px-1 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-sm">
                          {count > 9 ? '9+' : count}
                        </span>)}
                    </Link>);
            })}
              </>) : (<div className="flex items-center gap-6">
                {publicNavLinks.map((link) => (<button key={link.path} onClick={() => handleNavClick(link.path)} className="text-sm font-medium text-gray-600 hover:text-black transition-colors">
                    {link.name}
                  </button>))}
              </div>)}
          </div>
          ...
          <div className="flex gap-4 items-center">
            {isAuthenticated ? (<div className="hidden md:flex items-center gap-4">
                <Link to="/notifications" className="relative p-2 text-gray-500 hover:text-black transition-colors hover:bg-gray-100 rounded-full">
                  <Bell size={20}/>
                  {unreadNotifications > 0 && (<span className="absolute top-1 right-1 h-3 w-3 bg-red-500 rounded-full border-2 border-white"></span>)}
                </Link>
                ...
                <button onClick={handleLogout} className="text-gray-400 hover:text-red-600 transition-colors p-2 hover:bg-red-50 rounded-full" title="Logout">
                  <LogOut size={18}/>
                </button>
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'max-h-96 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
          <div className="bg-white/90 backdrop-blur-md border border-gray-200/60 rounded-2xl shadow-xl p-2 space-y-1">
            {isAuthenticated ? (<>
                ...
                {navLinks.map((link) => (<Link key={link.path} to={link.path} className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-black transition-all" onClick={() => setIsMobileMenuOpen(false)}>
                    {link.icon}
                    {link.name}
                  </Link>))}
                <Link to="/notifications" className="flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-black transition-all" onClick={() => setIsMobileMenuOpen(false)}>
                  <Bell size={18}/>
                  Notifications
                  {unreadNotifications > 0 && (<span className="ml-auto bg-red-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">
                      {unreadNotifications}
                    </span>)}
                </Link>
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 transition-all text-left mt-2">
                  <LogOut size={18}/>
                  Logout
                </button>
              </>) : (<div className="p-2 space-y-1">
                {publicNavLinks.map((link) => (<button key={link.path} onClick={() => handleNavClick(link.path)} className="w-full text-left px-4 py-3 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-black transition-all">
                    {link.name}
                  </button>))}

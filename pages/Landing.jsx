...
const Landing = () => {
    ...
    return (<div className="bg-white relative">
      ...
      <section className="px-6 lg:px-12 py-12 lg:py-24 relative">
        <motion.div className="max-w-[1200px] mx-auto grid lg:grid-cols-2 gap-16 items-center" initial="hidden" animate="visible" variants={containerVariants}>
          <motion.div className="flex flex-col gap-8" variants={itemVariants}>
            {/* Badge */}
            <motion.div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/5 border border-borderLight text-primary text-xs font-bold uppercase tracking-wider w-fit" variants={itemVariants}>
            ...
            {/* Heading */}
            <motion.h1 className="text-5xl lg:text-7xl font-black leading-[1.1] tracking-tight text-textDark" variants={itemVariants}>
              Find your perfect space,
              <span className="text-muted"> with the perfect person.</span>
            </motion.h1>

            {/* Subheading */}
            <motion.p className="text-lg lg:text-xl text-muted leading-relaxed max-w-[540px]" variants={itemVariants}>
          ...
          <motion.div className="relative" variants={itemVariants}>
            <motion.div className="absolute -inset-4 bg-muted/5 blur-2xl rounded-[2rem]" style={{ opacity: scrollY > 100 ? 0.5 : 0.2 }}></motion.div>
            <motion.div className="relative border border-borderLight rounded-3xl overflow-hidden shadow-sm" whileHover={{ y: -10 }} transition={{ duration: 0.3 }}>
              ...
              <motion.div className="absolute bottom-6 left-6 right-6 bg-white border border-borderLight rounded-2xl p-4 shadow-xl" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.6 }}>
      ...
      <section className="px-6 lg:px-12 py-24 bg-card border-y border-borderLight" id="features">
        <motion.div className="max-w-[1200px] mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants}>
        ...
        ].map((feature, i) => (<motion.div key={i} className="group p-8 rounded-2xl bg-white border border-borderLight hover:border-primary transition-all shadow-sm" variants={itemVariants} whileHover={{ y: -5 }}>
                <motion.div className="size-16 rounded-full bg-primary/5 flex items-center justify-center mb-6 group-hover:bg-primary/10 transition-all" whileHover={{ scale: 1.1, rotate: 5 }}>
      ...
      <section className="px-6 lg:px-12 py-24 bg-white" id="how-it-works">
        <motion.div className="max-w-[960px] mx-auto" initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants}>
          <motion.h2 className="text-3xl font-black mb-12 text-center lg:text-left text-textDark" variants={itemVariants}>
        ...
        ].map((item, i) => (<motion.div key={i} className="grid grid-cols-[80px_1fr] gap-x-8 pb-16" variants={itemVariants}>
                <div className="flex flex-col items-center">
                  <motion.div className={`size-14 rounded-full flex items-center justify-center z-10 ${i < 2
                ? 'bg-primary text-white shadow-sm'
                : 'bg-white border-2 border-primary text-primary'}`} whileHover={{ scale: 1.15, rotate: 5 }} transition={{ type: 'spring', stiffness: 400, damping: 10 }}>
      ...
      <section className="px-6 lg:px-12 py-24 bg-card border-t border-borderLight" id="testimonials">
          ...
          <motion.div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8" variants={containerVariants}>
            ...
            },
            {
        ...
        ].map((testimonial, i) => (<motion.div key={i} className="bg-white p-8 rounded-2xl border border-borderLight flex flex-col gap-6 shadow-sm hover:border-primary transition-all" variants={itemVariants} whileHover={{ y: -5 }}>
                ...
                <div className="flex items-center gap-4 pt-4 border-t border-borderLight">
                  <img alt={testimonial.name} className="size-10 rounded-full object-cover" src={testimonial.image}/>
                  <div>

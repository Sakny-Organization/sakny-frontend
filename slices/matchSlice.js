...
const toCards = (items, myProfile) =>
  (Array.isArray(items) ? items : []).map((p) => profileToCard(p, myProfile));
...
export const fetchRecommendations = createAsyncThunk(
  ...
  async (limit = 4, { getState }) => {
    const myProfile = getState().auth.user;
    const items = await matchService.getRecommendedMatches(limit);
    return toCards(items, myProfile);
  },
);
...
export const fetchMatchById = createAsyncThunk(
  ...
  async (id, { getState }) => {
    const myProfile = getState().auth.user;
    const profile = await matchService.getMatchById(id);
    return profile ? profileToCard(profile, myProfile) : null;
  },
);
